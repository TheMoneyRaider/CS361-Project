...
class Database:
  def __init__(self, path, path2):
    ...
    for x in range(self.num_books):
        self.books.append(Book(file.readline(),file.readline(),file.readline(),file.readline(),file.readline(),file.readline(),file.readline()))
    file.close()
    file = open(path2, 'r')
    self.settings = []
    for x in range(14):
  ...
  def append_book(self,book):
    ...
    for x in range(self.num_books):
        ...
        file.write(self.books[x].progress)
        file.write(self.books[x].status)
    file.write("\n")
    file.close()
  ...
  def print_settings(self):
      ...
      if(self.settings[13]==0):
        ...
        string4="[20]"
      print(f"(B): Books per page   {string1} {string2} {string3} {string4}")

  def get_page_size(self):
     if self.settings[10]==1:
        return 5
     if self.settings[11]==1:
        return 10
     if self.settings[12]==1:
        return 15
...
def read_book(database):
    ...
    book = Book(file.readline(),file.readline(),file.readline(),file.readline(),file.readline(),file.readline(),file.readline())
    file.close()
    while(repeat==0):
